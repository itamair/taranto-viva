/**
 * @file
 * Provides UI/UX progressive enhancements on Taranto Viva's theme settings by
...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.taranto_viva.colorSchemes;
  ...
  function synchronizeInputs(changedInput, inputToSync) {
    ...
    changedInput.setAttribute('data-taranto_viva-custom-color', changedInput.value);
    inputToSync.setAttribute('data-taranto_viva-custom-color', changedInput.value);
  ...
  function setColorScheme({ target }) {
    ...
    if (selectedColorScheme) {
      ...
      document
        .querySelectorAll(`input[data-taranto_viva-custom-color]`)
        .forEach((input) => {
          input.value = input.getAttribute('data-taranto_viva-custom-color');
        });
    }
  }
  ...
  /**
   * Initializes Taranto Viva theme-settings color picker.
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      'data-taranto_viva-custom-color',
      textInput.getAttribute('data-taranto_viva-custom-color'),
    );
  ...
  /**
   * Taranto Viva Color Picker behavior.
   *
   * @type {Drupal~behavior}
   * @prop {Drupal~behaviorAttach} attach
   *   Initializes color picker fields.
   */
  Drupal.behaviors.taranto_vivaColorPicker = {
    attach: () => {
      const colorSchemeSelect = once(
        'taranto_viva-color-picker',
        '[data-drupal-selector="edit-color-scheme"]',
      );
      ...
      const colorTextInputs = once(
        'taranto_viva-color-picker',
        '[data-drupal-selector="taranto_viva-color-picker"] input[type="text"]',
      );
